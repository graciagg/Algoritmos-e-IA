...
### ---- Pasos previos ----
### ---- 0. Cargar librerias e instalar librerias ---- 
...
# ---- 0.1 Pasos previos : Asignar dierctorio y cargar los datos y librerias ----
 ...
 Al ser 5 integrantes en el equipo se establece un "path" para cada uno, asi se facilita el trabajo en grupo
 
YA NO HACE FALTA: Si trabajas desde el repo de git, tienes los datos en la misma carpeta
```{r}
'path_Maria.Luisa <-
path_Sara <-'
path_Gracia <-
...
clases <- read.csv("C:\\Users\\Guille\\Desktop\\MASTER\\ASIGNATURAS\\PRIMER CUATRI\\IA\\actividades\\a3\\classes.csv", sep = ";", col.names = c("SampleID","Clase"))                                                 # Cargo el csv "clases" 
df <- read.csv("C:\\Users\\Guille\\Desktop\\MASTER\\ASIGNATURAS\\PRIMER CUATRI\\IA\\actividades\\a3\\gene_expression.csv", sep = ";", col.names = read_lines("C:\\Users\\Guille\\Desktop\\MASTER\\ASIGNATURAS\\PRIMER CUATRI\\IA\\actividades\\a3\\column_names.txt"), row.names = clases$SampleID) # cargo mi dataframe de trabajo
...
scaled_df$Class <- clases$Clase
```

 Veo que no hay ningun NA, por lo que no tenemos que imputar ningún dato. Escalamos los datos y guardamos en un nuevo df llamado df_scale 
...
### 1.2. t-SNE
Probamos con este por que, al preservar las distancias locales en el nuevo espacio, puede ser útil para aplicar métodos de ML después, ya que no deforma tanto los datos, solamente los aplana.
...
```{r}
##### 2.CLUSTERIZACIÓN
matriz_dist<-dist(tsne_result) #Creamos la matriz de distancias cogiendo como base de datos la obtenida en 
                              #el método óptimo de reducción de dimensionalidad

#Implementamos métodos jerárquicos debido a que los no jerárquicos son más útiles en dataset pequeños.
#Elegimos los modelos complete y ward ya que al hacer diversas pruebas son los que muestran unos clústers más definidos
#y proporcionados. 
...
fviz_nbclust(tsne_result, kmeans, method = "wss")     #Comprobamos el número de k más adecuado

kmeans.result<-kmeans(tsne_result, centers = 3, iter.max=100)

fviz_cluster(kmeans.result, tsne_result)    #Graficamos

#Como suponíamos al principio de este apartado, al haber tantos datos a pesar de haber aplicado el modelo de 
#reducción de dimensionalidad, los datos se encuentran muy amontados y no aportan información útil en este método.
```
...
```{r}
set.seed(12345)                                                                 # seteo la semilla y creo el modelo
