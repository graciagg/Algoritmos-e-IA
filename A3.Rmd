...
path_Gorka <- "~/Desktop/UNIR/Algoritmos e Inteligencia Artificial/Actividad 3"

setwd(path_Gracia)
```
## ---- 0.2 Pasos previos : Cargar datos y comprobacion inicial ----

Cargamos los datos y comprobamos que ninguna columna nos de datos iguales a 0. En caso de tener variables con valores nulos, los excluiremos del df inicial.
...
Preprocesado y comprobacion previa, con escalado de los datos.
```{r}
any(is.na(df))
...
Elegimos los modelos complete y ward ya que al hacer diversas pruebas son los que muestran unos clusters más definidos y proporcionados.
...
legend("bottomright", legend = c(lda_legend, rda_legend, knn_legend, svmk_rad_legend,svmk_lin_legend,RForest_legend,NBayes_legend),
       col = c("deepskyblue2","darkolivegreen2", "orange2","firebrick1","purple","yellow","darkblue"), lwd = 2, cex = 0.5)
```



### ---- Obtener valores F1-Score ----
...
### ¿Qué método habéis escogido para llevar a cabo la imputación de los datos? Razonad vuestra respuesta. ¿Habéis llevado a cabo algún otro tipo de procesamiento? Razonad vuestra respuesta.

Lo primero fue, como siempre, establecer directorios personales para poder trabajar de forma continua.
Para construir nuestro dataset partíamos de 3 archivos:
 1. clases (formato.csv)
 2. gene_expresion (formato.csv)
 3. column_names (formato.txt)

Primero hemos leído los archivos .csv (teniendo en cuenta que los nombres de las columnas de nuestro dataset se encontraban en el fichero 'column_names').
Para la limpieza y preprocesado de nuestros datos, hemos comprobado qué columnas tenían valores faltantes y las hemos eliminado de nuestro df (columnas que sumaban 0). También hemos comprobado que no teníamos datos faltantes, por lo que no hemos necesitado imputar ningún dato. Por último, hemos escalado nuestros datos para asegurarnos que, en los algoritmos posteriores, todas las variables tienen la misma importancia y evitar que las diferencias en sus escalas afectaran negativamente a los distintos modelos utilizados.
...
Hemos comenzado por el que quizás es uno de los algoritmos de reducción más usados en estos ámbitos, el análisis de componentes principales *(PCA)*. Este  método es una técnica lineal de reducción de dimensionalidad cuyo objetivo es transformar un conjunto de datos en un nuevo conjunto de variables no correlacionadas, llamadas componentes principales, tratando de maximizar la varianza de los datos. Este método además de mostrar los resultados de una forma clara y concisa, ayuda a prevenir el overfitting en el modelo.

El PCA no pudo separar bien los grupos, por lo que asumimos que las relaciones entre clases no son lineales, y para el segundo método escogimos el *t-SNE* ya que, como dijimos anteriormente, este algoritmo preserva las distancias locales en el nuevo espacio y no deforma  tanto los datos. Este método, pese a tener un componente estocástico, captura muy bien las complejas relaciones polinómicas entre las características y general es muy eficiente a la hora de identificar la estructura de los datos.

### ¿Cuál es el motivo por el cual habéis seleccionado estas técnicas de clusterización? En ambos casos, ¿qué aspectos positivos y negativos tienen cada una? En el caso de la clusterización, ¿podéis afirmar con certeza que los clústeres generados son los mejores posibles?

El primer método que utilizamos fueron los *clusters jerárquicos aglomerativos*, los cuales se basan en un enfoque conocido como bottom-up, donde cada observación constituye su propio grupo, y se va fusionando con el resto a medida que se va subiendo de nivel. Escogimos este método para identificar patrones de expresión génica similares entre las muestras.
Como linkage usamos:
...
La mayor ventaja de este modelo radica en que no requiere un número predeterminado de clusters, ya que el propio método los va construyendo. Sin embargo, presenta algunos inconvenientes: computacionalmente es bastante costoso, y los grupos obtenidos se ven muy afectados por el ruido que puedan contener los datos.

Después aplicamos *clusterización no-jerárquica k-means*. Este método asume que los datos se encuentran en grupos separados y no superpuestos. Esto significa que cada punto de datos pertenece a un solo grupo y no se engloban grupos dentro de otros. Al ser la elección del número de centroides arbitraria (basándonos en la técnica del codo), puede que no sea el más óptimo para detectar correctamente las relaciones entre pacientes, haciendo así más difícil obtener los mejores clusters posibles para nuestros datos.
...
En el apartado de aprendizaje supervisado implementamos un total de 8 métodos, ya que cada uno de los algoritmos utilizados se basa en unos principios diferentes. De este modo podemos comparar el performance de distintos modelos, y ver cuál se ajusta más a nuestros datos.

Los modelos usados han sido:
- Análisis discriminante lineal (LDA)
- Análisis Discriminante Regularizado (RDA)
- K Nearest Neighbor (K-NN)
- Suport Vector Machine con 3 tipos de kernel (Gaussiano, Lineal y Polinómico)
- Random Forest
- Naive Bayes

Todos los métodos de aprendizaje supervisado nos han proporcionado muy buenos resultados en cuanto a sensibilidad, especificidad y exactitud, medidas a través de las matrices de confusión proporcionada por cada método. 
Los modelos que han obtenido los mejores resultados en todos los estadísticos han sido el k-NN, LDA y RDA.

Las curvas ROC resultan poco representativas en este caso, ya que los métodos nos proporcionan resultados tan parecidos entre ellos que están se ven superpuestas unas con otras.
...
Intentamos implementar una reduccion LASSO multinomial pero algo fallaba en el comando y no logramos hacerla bien. Pensamos que podría ser un enfoque curioso para evitar colinealidad y variables "poco relevantes", sin embargo, el desempeño de los modelos con la totalidad de los datos ha sido bastante bueno.
 ...
 *1. LDA:*
    ...
    -Si el número de observaciones es bajo y la distribución de las variables predictoras es aproximadamente normal en cada una de las clases, el LDA es más estable que la regresión logística.
    Desventajas:
...
*3. kNN:*
    ...
    -Selección arbitraria del valor k
    -Clasificación lenta
...
*4-5-6: SVM Suport Vector Machine:*
    ...
    -Kernel lineal: solo admite hiperplanos lineales pero es mas rápido y menos costoso
    -Kernel gaussiano: pueda mapear los datos a espacios de características de mayor dimensión utilizando una función de base radial, es más costoso computacionalmente
    -Kernel polinomico: no se limita a hiperplanos lineales, pero igual que el anterior, es más lento y más costoso
...
*8. Naive Bayes:*
    ...
    Desventajas:
    -Se basa en la suposición de independencia entre variables
...
Generalmente, la opción más adecuada para procesar datos de expresión génica sería la *red de perceptrones*, ya que son muy adecuadas para aprender patrones complejos de relación entre variables, algo intrínseco en la naturaleza de este tipo de datos. Una de sus mayores ventajas es que pueden aprender tanto relaciones lineales como no lineales entre los genes y las clases (como en el dataset de esta actividad, por ejemplo, para clasificar tipos de cáncer según la expresion génica del paciente).

El resto de redes presentadas están diseñadas para trabajar con otros tipos de datos:
- Redes convolucionales: imágenes y otros datos espaciales
- Redes recurrentes: datos temporales
- Redes de grafos: datos que tienen conexiones entre elementos. 
